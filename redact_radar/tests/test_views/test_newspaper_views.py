...
from redact_radar.views import NewspaperListView, NewspaperCreateView, NewspaperUpdateView, NewspaperDeleteView
...
class PublicNewspaperTests(TestCase):
    def test_newspaper_create_update_delete_login_required(self):
        newspaper_create = self.client.get(reverse("redact_radar:newspaper-create"))
        newspaper_update = self.client.get(reverse("redact_radar:newspaper-update", args=[1]))
        newspaper_delete = self.client.get(reverse("redact_radar:newspaper-delete", args=[1]))
...
class PrivateNewspaperTests(TestCase):
    ...
    def test_newspaper_update_login_required(self):
        newspaper_update = self.client.get(reverse(
            "redact_radar:newspaper-update", args=[self.newspaper.id])
        )
